...
const Home: React.FC = () => {
  ...
  return (
    <>
      ...
      <main className="max-w-[850px] mx-auto pt-10 text-white text-sm">
        <div className="flex items-center">
          Today's date
          <FiChevronRight className="mx-2" />
          <span>{new Date().toLocaleDateString()}</span>
        </div>
        <h1 className="mt-2 text-3xl font-bold">Vote Now</h1>
        <p className="text-neutral-400">~ Click your favorite brainrot</p>
        ...
        {isLoading ? (
          ...
          <motion.div layout className="flex flex-wrap gap-8 mt-10">
            {sortedMemes.map((meme, i) => (
              <motion.div
                ...
                className="relative w-[250px] h-[250px] max-md:w-[200px] max-md:h-[200px] bg-black border-4 border-neutral-700 rounded-xl overflow-hidden cursor-pointer"
              >
                <img
                  src={meme.image}
                  alt={`Meme ${meme.id}`}
                ...
                <div className="absolute bottom-0 w-full h-24 bg-gradient-to-t from-black to-transparent p-2 flex flex-col items-center justify-end">
                  <p className="text-xs text-white">Click! Click!</p>
